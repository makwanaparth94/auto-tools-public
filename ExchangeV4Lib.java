...
public class ExchangeV4Lib extends AppBase {
    ...
    public ExchangeV4FilterRequest getToolboxFilterWhereGroupIsNone() {
        //Filter for all types
        List<ExchangeV4FilterDynamicRequest> dynamicRequestList = buildV4DynamicFilterList(
            "model",
            Stream.of(ExchangeModuleTypes.values())
                .map(ExchangeModuleTypes::name)
                .collect(Collectors.toList())
        );

        // Step 2: Add condition: group EQ ""
        dynamicRequestList.add(
            ExchangeV4FilterDynamicRequest.builder()
                .field("group")
                .expressions(List.of(
                    ExchangeV4FilterDynamicExpressionsRequest.builder()
                        .operator("EQ")
                        .value("")
                        .build()
                ))
        ...
        return createExchangeV4FilterRequest(100, 0, "name", "", dynamicRequestList);
    }
